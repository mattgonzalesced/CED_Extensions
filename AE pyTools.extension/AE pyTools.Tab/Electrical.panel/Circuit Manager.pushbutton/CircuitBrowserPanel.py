...
def _bootstrap_lib_root(start_dir):
    current = os.path.abspath(start_dir)
    while True:
        candidate = os.path.join(current, "CEDLib.lib")
        if os.path.isdir(candidate):
            lib_root = os.path.abspath(candidate)
        ...
        if not parent or parent == current:
            break
        current = parent
    fallback = os.path.abspath(os.path.join(start_dir, "..", "..", "..", "..", "..", "CEDLib.lib"))
...
class MarkExistingActionWindow(forms.WPFWindow):
    ...
    def set_new_clicked(self, sender, args):
        ...
        if not target_rows:
            self._sync_button_states()
            return
        self._preview_callback(target_rows, "new", set_notes, clear_wire, clear_conduit)
        for row in target_rows:
            row.is_checked = False
        self._refresh_grid(refresh_items=False)
    ...
    def _preview_mark_existing_rows(self, rows, mode, set_notes, clear_wire, clear_conduit):
        ...
        for row in rows:
            ...
            if is_new_mode:
                ...
                row.new_wire = "-" if (clear_wire or clear_conduit) else row.current_wire
            row.recompute_state()

    def _apply_mark_existing_rows(self, rows, mode, set_notes, clear_wire, clear_conduit):
        changed_ids = [x.circuit_id for x in rows if x.is_enabled and x.is_changed]
        if not changed_ids:
            forms.alert("No circuits are marked for modification.", title=TITLE)
            return False
        mode_text = str(mode or "existing").lower()
