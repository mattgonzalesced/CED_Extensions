...
def main():
    ...
    # Get all available CAD profile names from the YAML
    available_profiles = sorted(repo.cad_names())
    if not available_profiles:
        forms.alert("No CAD profiles found in {}.".format(yaml_label), title=TITLE)
        return

    # Show profile selection dialog
    selected_profiles = forms.SelectFromList.show(
        available_profiles,
        title="Select Profiles (CAD Names) to Place",
        multiselect=True,
        button_name="Continue"
    )
    ...
    if results.get("placed", 0) == 0:
        ...
        msg_lines.append(" - Selected categories and profiles match the equipment definitions.")
