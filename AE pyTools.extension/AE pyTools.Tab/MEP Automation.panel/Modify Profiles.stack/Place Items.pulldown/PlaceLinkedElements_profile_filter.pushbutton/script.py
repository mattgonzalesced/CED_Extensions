# -*- coding: utf-8 -*-
"""Place selected equipment definitions on matching linked elements using the active YAML store."""
...
TITLE = "Place Linked Elements (Profile Filter)"
...
def _build_truth_group_entries(equipment_defs, available_cads):
    available = {name for name in (available_cads or []) if name}
    groups = {}
    covered = set()
    for entry in equipment_defs or []:
        cad_name = (entry.get("name") or entry.get("id") or "").strip()
        if not cad_name or cad_name not in available:
            continue
        truth_id = (entry.get(TRUTH_SOURCE_ID_KEY) or entry.get("id") or cad_name).strip() or cad_name
        display = (entry.get(TRUTH_SOURCE_NAME_KEY) or cad_name).strip() or cad_name
        group = groups.setdefault(truth_id, {"display": display, "members": []})
        if cad_name not in group["members"]:
            group["members"].append(cad_name)
        covered.add(cad_name)

    for cad_name in sorted(available):
        if cad_name in covered:
            continue
        groups[cad_name] = {"display": cad_name, "members": [cad_name]}
    ...
    entries = []
    for group_key in sorted(groups.keys(), key=lambda key: (groups[key].get("display") or key).lower()):
        info = groups[group_key]
        label = info.get("display") or group_key
        if display_counts.get(label, 0) > 1:
            label = "{} [{}]".format(label, group_key)
        members = info.get("members") or []
        entries.append({
            "label": label,
            "group_key": group_key,
            "display": info.get("display") or group_key,
            "members": sorted(set(members), key=lambda value: value.lower()),
        })
    return entries
...
def main():
    ...
    group_entries = _build_truth_group_entries(data.get("equipment_definitions") or [], equipment_names)
    if not group_entries:
        forms.alert("No profile groups found in {}.".format(yaml_label), title=TITLE)
        return

    xaml_path = os.path.join(LIB_ROOT, "UIClasses", "ProfileSelectionWindow.xaml")
    ok, selected_entries = show_profile_selection_window(xaml_path, group_entries)
    if not ok:
        return
    if not selected_entries:
        forms.alert("No profiles selected. Nothing to place.", title=TITLE)
        return

    selected_group_labels = []
    selected_cad_set = set()
    for entry in selected_entries:
        selected_group_labels.append(entry.get("label") or entry.get("display") or entry.get("group_key"))
        selected_cad_set.update(entry.get("members") or [])

    equipment_names = [name for name in equipment_names if name in selected_cad_set]
    if not equipment_names:
        forms.alert("None of the selected profiles are available in {}.".format(yaml_label), title=TITLE)
        return
    ...
    if not placeholders:
        forms.alert(
            "No linked elements were found whose names match selected equipment definitions. "
    ...
    if not rows:
        summary = [
            ...
            "Selected profile groups: {}.".format(len(selected_group_labels)),
            "Selected profiles: {}.".format(len(equipment_names)),
        ]
