...
def main():
    ...
    for linked_name, count in linked_counts.items():
        ...
        if score < MIN_PARTIAL_SCORE:
            continue
        overlap_count, _small_cov, large_cov = _token_overlap_metrics(linked_name, best)
    ...
    items = []
    linked_header = "NEW LINKED NAME (create)"
    profile_header = "EXISTING PROFILE (source)"
    count_header = "Count"
    linked_width = max(
        len(linked_header),
    ...
    linked_width = min(linked_width, 64)
    ...
    divider_label = "{}-+-{}-+-{}".format("-" * linked_width, "-" * profile_width, "-" * 5)
    ...
    for linked_name, best, count in missing:
        linked_cell = _truncate_text(linked_name, linked_width)
        profile_cell = _truncate_text(best, profile_width)
        count_cell = "x{}".format(count) if count > 1 else ""
