...
from Autodesk.Revit.DB import FilteredElementCollector, Electrical, Transaction, BuiltInCategory, BuiltInParameter, FamilyInstance, ElementId
# Import reusable utilities
from Snippets._elecutils import get_panel_dist_system, get_compatible_panels

# Use get_panel_data and get_compatible_panels as needed in the script

# Get the current document
doc = __revit__.ActiveUIDocument.Document


# Helper function to get selected electrical circuits

def get_circuits_from_selection(include_electrical_equipment=True, debug=False):
    """
    ...
    Args:
        ...
        debug (bool): If True, prints debug information for deeper inspection.
    ...
    for element_id in selection:
        ...
        # If the element is a FamilyInstance
            ...
            if element.Category.Id == ElementId(
                ...
                if debug:
                    print("Skipping electrical equipment: %s" % element.Id)
                continue  # Skip this electrical equipment family

            mep_model = element.MEPModel
            if mep_model:
                # Handle connectors for primary circuits first
                connector_iterator = mep_model.ConnectorManager.Connectors.ForwardIterator()
                ...
                while connector_iterator.MoveNext():
                    ...
                    if connector_info and connector_info.IsPrimary:
                        ...
                        if debug:
                            print("Primary connector found on family instance: %s" % element.Id)
                        for ref in connector.AllRefs:
                            ref_owner = ref.Owner
                            if ref_owner and isinstance(ref_owner, Electrical.ElectricalSystem):
                                if debug:
                                    print("Adding circuit from primary connector's owner: %s" % ref_owner.Id)
                    ...
                    if electrical_systems:
                        for circuit in electrical_systems:
                            ...
                            if element.Category.Id == ElementId(BuiltInCategory.OST_ElectricalEquipment):
                                if circuit.BaseEquipment is None or circuit.BaseEquipment.Id != element.Id:
                                    if debug:
                                        print("Adding feeder circuit: %s" % circuit.Id)
                                    circuits.append(circuit)
                                else:
                                    if debug:
                                        print("Omitting branch circuit: %s" % circuit.Id)
                            else:
                                if debug:
                                    print("Adding circuit from family: %s" % circuit.Id)
                                circuits.append(circuit)

    # If no circuits are found, exit the script
    if not circuits:
        if debug:
            print("No circuits found. Exiting script.")
        script.exit()
...
def get_compatible_panels(selected_circuit):
    ...
    all_panels = FilteredElementCollector(doc).OfCategory(BuiltInCategory.OST_ElectricalEquipment).WhereElementIsNotElementType().ToElements()
...
def main():
    ...
    target_panel_name = forms.ask_for_one_item(panel_names, title="Select Target Panel", prompt="Choose a panel to move the circuits to:")
