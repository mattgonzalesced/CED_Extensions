...
class FluidType(Element):
    ...
    def in_use_dict(doc=revit.doc):
        ...
        """
        result = {}
        for system in DB.FilteredElementCollector(doc).OfClass(
            DB.Plumbing.PipingSystemType
        ):
            rpw_system = Element(system)
            rpw_fluid_type = Element.from_id(system.FluidType)
            result[rpw_system.name] = {
                "name": rpw_fluid_type.name,
                "temperature": rpw_system.FluidTemperature,
            }
        return result
...
class PipingSystemType(Element):
    ...
    def read_from_csv(csv_path=None):
        if not csv_path:
            csv_path = forms.pick_file(file_ext="csv", title="Select File")
        ...
        for row in file_reader:
            ...
            if row_len < 3:
                print(
                    "Line {} is invalid, less than 3 column".format(
                        file_reader.line_num
                    )
                )
            ...
            shared_parameter_list.append(
                SharedParameter(name, group, parameter_type, visible, guid)
            )
