...
_GROUP_SIMPLE = {
    ...
    "electricalloads": GroupTypeId.ElectricalLoads if hasattr(GroupTypeId,
                                                              "ElectricalLoads") else GroupTypeId.Electrical,
...
class Runner(Form):
    def __init__(self):
        ...
        # Excel
        self.lblX = Label(Text="Excel");
        self.lblX.Location = Point(10, 20)
        self.txtX = TextBox();
        self.txtX.Location = Point(140, 18);
        self.txtX.Size = Size(640, 22)
        self.btnX = Button(Text="Browse");
        self.btnX.Location = Point(790, 16);
        self.btnX.Click += self.pick_xlsx

        # Shared Params
        self.lblS = Label(Text="Shared Params");
        self.lblS.Location = Point(10, 55)
        self.txtS = TextBox();
        self.txtS.Location = Point(140, 53);
        self.txtS.Size = Size(640, 22)
        self.btnS = Button(Text="Browse");
        self.btnS.Location = Point(790, 51);
        self.btnS.Click += self.pick_sp

        # Circuits
        self.lblC = Label(Text="Circuits (1-5):");
        self.lblC.Location = Point(10, 90)
        self.cboC = ComboBox();
        self.cboC.Location = Point(140, 88);
        self.cboC.DropDownStyle = ComboBoxStyle.DropDownList
        for i in range(1, 6): self.cboC.Items.Add(str(i))
        self.cboC.SelectedIndex = 0

        # Multi-family
        self.lblF = Label(Text="Families (.rfa)");
        self.lblF.Location = Point(10, 130)
        self.txtF = TextBox();
        self.txtF.Location = Point(140, 128);
        self.txtF.Size = Size(640, 22);
        self.txtF.ReadOnly = True
        self.btnF = Button(Text="Browse");
        self.btnF.Location = Point(790, 126);
        self.btnF.Click += self.pick_rfAs
        self._family_paths = []

        # Buttons
        self.btnRun = Button(Text="Run");
        self.btnRun.Location = Point(300, 190);
        self.btnRun.Click += self.run
        self.btnClose = Button(Text="Close");
        self.btnClose.Location = Point(370, 190);
    ...
    def pick_sp(self, s, a):
        dlg = OpenFileDialog();
        dlg.Title = "Select Shared Params";
        dlg.Filter = "Shared Params (*.txt)|*.txt|All Files (*.*)|*.*";
        dlg.CheckFileExists = True
        if dlg.ShowDialog() == DialogResult.OK: self.txtS.Text = dlg.FileName

    def pick_rfAs(self, s, a):
        dlg = OpenFileDialog();
        dlg.Title = "Select Families (.rfa)";
    ...
    def run(self, s, a):
        ...
        if not (os.path.isfile(xlsx) and os.path.isfile(sp)):
            log("[ERROR] Pick valid .xlsx and .txt.");
            return
        ...
        try:
            ...
            log("[ERROR] Prep failed: {}".format(e));
            return
...
def try_set_formula(fm, fp, name, formula, failed):
    """Set the formula for a parameter, but only override with the Apparent Load expression
    when the target parameter *looks like* an Apparent Load field. Everything else keeps
    the Excel-provided formula (so Integer params like Number of Poles won't get a real-valued formula)."""
    try:
        # Default to whatever Excel specified
        target_formula = formula or ""

        # Only for Apparent Load targets do we inject the Power Connection logic
        if _looks_like_apparent_load(name):
            pwr_param = get_param(fm, "Power Connection Required")
            if pwr_param:
                # If Type/Instance classification matches we can reference it directly
                if bool(pwr_param.IsInstance) == bool(fp.IsInstance):
                    target_formula = (
                    ...
                    # Fallback: avoid illegal cross reference (Type↔Instance) by using unconditional formula
                    target_formula = "(Circuit 1 Voltage_CED * Circuit 1 FLA_CED) * (if(Circuit 1 Phase_CED = 3, 1.732051, 1))"
            else:
                # Checkbox not present -> unconditional apparent-load formula
                target_formula = "(Circuit 1 Voltage_CED * Circuit 1 FLA_CED) * (if(Circuit 1 Phase_CED = 3, 1.732051, 1))"

        # If nothing to set, bail quietly
        if not target_formula:
            return False

        # Keep your existing special-case rewrite (no change)
...
def filter_by_circuits(rows, n):
    ...
    for r in rows:
        ...
        if cg is None or (1 <= cg <= n): out.append(r)
    log("[INFO] Rows after circuit filter (<= {}): {}".format(n, len(out)))
    return out
...
def apparent_targets(r):
    """Return all names we should consider for sorting from this Excel row.
       Includes the exact name and the de-prefixed base name (if different)."""
...
def add_and_update_params(fam_doc, rows, sp_byname, sp_byguid):
    ...
    try:
        for r in rows:
            ...
            if (r.get("FamOrShared") or "").strip().lower().startswith("family"):
                try:
                    fp = fm.AddParameter(name_target, group, spec, inst);
                    added.append(name_target)
                    if try_set_formula(fm, fp, name_target, formula, f_failed): f_set.append(name_target)
                except Exception as e:
            ...
            ext = None;
            g = parse_guid(r.get("Guid") or "")
            if g and g in sp_byguid:
                ext = sp_byguid[g]
            else:
                for (grp, nm), d in sp_byname.items():
                    if nm == name_target: ext = d; break
            if not ext:
                add_failed.append("{} (shared add skipped: no def)".format(name_target));
                continue

            try:
                fp = fm.AddParameter(ext, group, inst);
                added.append(name_target)
                if try_set_formula(fm, fp, name_target, formula, f_failed): f_set.append(name_target)
            except Exception as e:
...
def reorder_params(fam_doc, rows):
    ...
    # Build desired (cg, so) map from Excel; accept row if either is present
    for r in rows:
        cg = r.get("CircuitGroupNum")
        so = r.get("SortOrderNum")
        for name in apparent_targets(r):
            if cg is not None or so is not None:
                prefs[name] = (cg, so)
    ...
    def key(p):
        n = p.Definition.Name
        # try exact; if not found, try base (de-prefixed) match
        cg_so = prefs.get(n)
        if cg_so is None:
            cg_so = prefs.get(_strip_prefix_local(n))
        cg, so = cg_so if cg_so is not None else (None, None)
        return (
            cg if isinstance(cg, int) else BIG,
            so if isinstance(so, int) else BIG,
            base.get(n, BIG)  # preserve original relative order for "unknowns"
        )
    ...
    t.Start()
    try:
        # Revit API (2024/2025): reorder whole set in one shot
        fm.ReorderParameters(sorted_params)
        t.Commit()
        log("[INFO] Reorder complete (by '{}' then '{}').".format(HDR_CIRCUIT, HDR_SORT))
    except Exception as e:
        t.RollBack()
...
def silent_open_family(path):
    # Opens into memory; not made visible. Uses ModelPath + OpenOptions.
    mp = ModelPathUtils.ConvertUserVisiblePathToModelPath(path)
    opts = OpenOptions();
    opts.Audit = False
    return app.OpenDocumentFile(mp, opts)  # opens in memory, no UI.


def _attach_dialog_silencer():
    """Temporarily auto-OK TaskDialogs (e.g., 'pyRevit – Document Opened: ...')."""
        ...
        try:
            if e.GetType().Name == 'TaskDialogShowingEventArgs':
                ...
                if ("document opened" in msg.lower()) or ("pyrevit" in msg.lower()) or ("pyrevit" in did.lower()):
                    try:
                        e.OverrideResult(int(TaskDialogResult.Ok))  # auto-press OK
                    except:
                        try:
                            e.OverrideResult(1)  # fallback OK code
                        except:
                            pass
        except:
            pass
    ...
    try:
        ...
        pass
    return _handler  # token for later removal
...
def process_family_file(path, rows, sp_byname, sp_byguid):
    ...
    try:
        ...
        # NEW: save edits now so the helper sees the updated file on disk
