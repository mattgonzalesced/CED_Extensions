...
class NoticeCollector(object):
    ...
    def __init__(self, circuit_name):
        ...
        self.items = []  # list of (level, category, message)

    def add(self, level, message, category="Calculation"):
        self.items.append((level.upper(), category, message))
    ...
    def categorized(self):
        ordered = ["Override", "Calculation", "Design", "Error"]
        buckets = {key: {"WARNING": [], "ERROR": []} for key in ordered}
        buckets["Other"] = {"WARNING": [], "ERROR": []}

        for level, category, msg in self.items:
            cat_key = category if category in buckets else "Other"
            if level not in buckets[cat_key]:
                buckets[cat_key][level] = []
            buckets[cat_key][level].append(msg)

        # Remove empty categories while preserving order
...
class CircuitBranch(object):
    ...
    def log_warning(self, msg, *args, **kwargs):
        ...
        self.notices.add("WARNING", formatted, category)
        if DEV_LOGGING:
    ...
    def log_error(self, msg, *args, **kwargs):
        category = kwargs.pop("category", "Calculation")
        formatted = msg.format(*args) if args else msg
        self.notices.add("ERROR", formatted, category)
        if DEV_LOGGING:
