# -*- coding: utf-8 -*-
"""
Proximity/clearance check for sprinklers vs nearby obstructions (beams, duct, lights, cable tray).
"""
...
def collect_hits(doc, options=None):
    ...
    if not sprinklers or not obstructions:
        return []

    hits = []
    for spr in sprinklers:
        spr_bbox = spr.get("bbox")
        if spr_bbox is None:
            continue
        for obs in obstructions:
            dist_ft = _bbox_distance(spr_bbox, obs.get("bbox"))
            if dist_ft is None:
                continue
...
def run_check(doc=None, show_ui=True, show_empty=False, options=None):
    ...
    if show_ui:
        hits = [
            {
                "a_id": item.get("sprinkler_id"),
                "a_label": item.get("sprinkler_label"),
        ...
        report_proximity_hits(
            ...
            subtitle="Sprinklers within {:.0f} inches of beams, duct, lights, or tray".format(
                threshold_inches
            ),
            hits=hits,
            columns=[
        ...
        if hits:
            forms.alert(
                "Found {} sprinkler(s) within {:.0f} inches of nearby obstructions.\n\n"
