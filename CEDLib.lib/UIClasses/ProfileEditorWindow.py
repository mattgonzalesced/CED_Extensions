...
class ProfileEditorWindow(forms.WPFWindow):
    ...
    def CreateOrphanButton_Click(self, sender, args):
        ...
        if not cad_name:
            forms.alert("Profile name cannot be empty.", title="Create Independent Profile")
            return
        lowered = cad_name.lower()
        for existing in self._profiles.keys():
            if (existing or "").strip().lower() == lowered:
                forms.alert("A profile named '{}' already exists.".format(cad_name), title="Create Independent Profile")
                return
    ...
    def _apply_read_only_state(self):
        ...
        for row in self._keynote_rows:
            for key in ("family", "type", "key_value", "key_description", "x", "y", "z", "rot"):
                box = row.get(key)
                if box:
                    box.IsReadOnly = read_only
    ...
    def _collect_tag_configs(self, rows):
        ...
        for row in rows:
            ...
            key_description_box = row.get("key_description")
            ...
            if panel_type == "_keynote_rows" and key_description_box is not None:
                key_description = (key_description_box.Text or u"").strip()
                if key_description:
                    parameters.pop("Keynote Text", None)
                    parameters["Keynote Description"] = key_description
            configs.append(
                TagConfig(
    ...
    def _add_tag_row(self, tag=None, target_list_name="TagList", storage_attr="_tag_rows"):
        ...
        if key_description_box is not None:
            key_description_box.Text = self._extract_keynote_description(tag)
        ...
        storage.append({
            ...
            "key_description": key_description_box,
            "x": x_box,
            "y": y_box,
            "z": z_box,
            "rot": rot_box,
    ...
    def _extract_keynote_description(self, tag):
        params = self._extract_tag_parameters(tag) or {}
