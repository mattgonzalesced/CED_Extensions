...
content_folder = r"DC\ACCDocs\CoolSys\CED Content Collection\Project Files\03 Automations\Walmart\Content"
# content_dir = os.path.join(user_folder,content_folder)
...
def show_sharepoint_sync_instructions(content_dir):
    output.freeze()
    output.print_md("## ⚠️ ACC Content Folder Not in Desktop Connector\n")

    output.print_md(
        "This tool cannot proceed because the required ACC Project is missing from Desktop Connector.\n\n"
        "The content must be loaded from the **CED Content Collection** ACC site."
    )
    output.print_md("🌐 [{}]({})".format("ACC Project Website", acc_web))

    output.print_md("## 🔧 How to fix this issue:\n")

    output.print_md(
        "1. Click the Desktop Connector tray icon on your taskbar\n"
        "2. Click **'Select Projects'** and add **CED Content Collection** from the **Coolsys** Directory.\n\n"
        "3. Once complete, confirm that the following folder now exists:\n"
        "    📂 [{}]({})\n\n"
    ...
    output.print_md("\n\n#### 🔍 Example: Add Project to Desktop Connector\n")
    ...
    output.print_md("\n\n#### 🗂 Example: ACC folder location in File Explorer\n")
...
def main():
    ...
    target_view = get_destination_view(doc)
    copy_view_id = DB.ElementId(System.Int64(9398925))
    copy_view = starter_doc.GetElement(copy_view_id)
    copy_view_name = copy_view.Name
    ...
    with DB.TransactionGroup(doc, "Load Shared Parameters and Content") as tg:
        ...
        delete_feeder_key_schedule(doc)
        set_leader_arrowheads_for_tags(doc)
